...
val_dataloader = dict(
    dataset=dict(
        ...
        data_root=data_root,
        metainfo=metainfo,
        ann_file='../../dataset/json_labels/data_day.json',
        data_prefix=dict(img='/root/AICITY2024_Track4/dataset/vip_cup_day/images/train')
        ))
test_dataloader = val_dataloader

val_evaluator = dict(  # Validation evaluator config
    type='CocoMetric',  # The coco metric used to evaluate AR, AP, and mAP for detection and instance segmentation
    ann_file='../../dataset/json_labels/data_day.json',  # Annotation file path
    metric=['bbox'],  # Metrics to be evaluated, `bbox` for detection and `segm` for instance segmentation
    format_only=True,
    outfile_prefix='./work_dirs/infer_day')
